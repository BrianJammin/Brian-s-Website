...
const addTask = (task) => {
  ...
  taskItem.innerHTML = `
    ...
    <label for="freeform">Project Details --></label><textarea id="freeform" name="freeform" rows="3" placeholder="Enter text here..." cols="30">
...
addTaskBtn.addEventListener('click', () => {
  ...
  const taskWithImg = `<img class="me-3" src="https://picsum.photos/80/80?random=${randomImgId}"> <span>${task}</span>`;
...
taskList.addEventListener('click', (e) => {
  ...
  if (target.matches('.task-check')) {
    ...
    if (target.checked) {
      ...
      label.classList.add('text-decoration-color: red');
    } else {
